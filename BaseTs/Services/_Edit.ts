...
export default class _Edit {
     ...
     */
    static setChildRows(upJson: any, childIdx: number, rows: any[]): any {
     ...
     * return void
     */
    static initVars(edit: any, box: JQuery): void {
        const fidTypes: string[] = [];
        const fidRadios: string[] = [];
        box.find(_Input.fidFilter()).each((i: number, item: HTMLElement) => {
            const obj = $(item);
            const j = i * 2;
        ...
        box.find('[data-type=file]').each((index: number, item: HTMLElement) => {
     ...
     */
    static async viewFileA(table: string, fid: string, elm: HTMLElement, key: string): Promise<void> {
        /*

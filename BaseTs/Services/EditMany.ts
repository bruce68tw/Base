...
export default class EditMany {
    ...
    private showErrors(rows: any[] | null): void {
        if (rows == null) return;
    ...
    private setChilds(childs: any[]): void {
        this[_Edit.Childs] = childs;
    }

    private initUrm(fids: string[]): void {
    ...
    private reset(rowsBox?: JQuery, forNew?: boolean): void {
    ...
    private _urmGetUpdJson(upKey: string): any {
        ...
        var fids = this.modeData;
        this._resetDeletes();
        this.rowsBox.find(':checkbox').each(function (this: HTMLElement) {
            var obj = $(this);
    ...
    private loadRowsBySys(rows: any[]): void {
        if (this.fnLoadRows) {
    ...
    private loadRowByBox(rowBox: JQuery, row: any, index: number): void {
        row.Index = index;
    ...
    private loadRowsByRsb(rows: any[] | null, reset?: boolean, rowsBox?: JQuery): void {
    ...
    private getKey(rowBox: JQuery): string {
    ...
    private getUpdJsonByRsb(upKey: string, rowsBox?: JQuery): any {
        var json: any = {};
    ...
    private getUpdRow(box: JQuery): any {
        return _Edit.getUpdRow(this, box);
    }

    private getUpdRows(upKey: string, rowsBox?: JQuery): any[] | null {
        ...
        var rows: any[] = [];
        var me = this;
        rowsBox.find(me.rowFilter).each(function (this: HTMLElement, idx: number, item: HTMLElement) {
            var box = $(item);
    ...
    private getDeletes(): string | null {
    ...
    private addRow(row?: any, rowsBox?: JQuery, newId?: number): any {
        row = row || {};
        rowsBox = this._getRowsBox(rowsBox);
    ...
    private deleteRow(key: string, rowBox?: JQuery): void {
    ...
    private deleteAll(): void {
        var me = this;
        this.rowsBox.find(this.rowFilter).each(function (this: HTMLElement) {
            var box = $(this);
            var key = _Input.get(me.kid, box);
    ...
    private dataAddFiles(levelStr: string, data: FormData, rowsBox?: JQuery): any {
        ...
        var fileJson: any = {};
        var fileIdx: any = {};
        rowsBox.find(me.rowFilter).each(function (this: HTMLElement, index: number, item: HTMLElement) {
            var tr = $(item);
    ...
    private rowSetFkey(row: any, fkey: string): void {
        if (row != null && _Edit.isNewRow(row, fkey))
            row[_Edit.DataFkeyFid] = fkey;
    }

    private rowsSetFkey(rows: any[] | null, fkey: string): void {
        if (rows != null) {
            for (var i = 0; i < rows.length; i++) {
                var row = rows[i];
    ...
    private rowsToNew(): void {
        if (this.rowsBox == null || this.rowsBox.length == 0) return;

        var me = this;
        me.newIndex = 0;
        me.rowsBox.find(me.rowFilter).each(function (this: HTMLElement) {
            me.newIndex--;
            _iText.set(me.kid, me.newIndex, $(this));
        });
    }

    private setNewIndex(index: number): void {
        this.newIndex = Math.abs(index) * -1;
    }

    private setNewIdByBox(box: JQuery, newId?: number): number {
        if (newId == null) {
            this.newIndex--;
            newId = this.newIndex;
        }
    ...
    private setSort(rowsBox?: JQuery): void {
        ...
        var me = this;
        rowsBox = this._getRowsBox(rowsBox);
        rowsBox.find(_Input.fidFilter(sortFid!)).each(function (this: HTMLElement, i: number, item: HTMLElement) {

        ...
        */
        ...
        public void AddImages(ref string srcTpl, List<WordImageDto> imageInfos)
        {
            ...
            foreach (var imageInfo in imageInfos)
            {
                ...
                var drawTpl = GetDrawTpl(srcTpl, ref drawStart, ref drawEnd, imageInfo.Code);
                if (string.IsNullOrEmpty(drawTpl))
                    continue;

                //find graphicData object(map to D.Graphic)
                int graphStart = 0, graphEnd = 0;
                var graphTpl = GetGraphDataTpl(drawTpl, ref graphStart, ref graphEnd);
                if (string.IsNullOrEmpty(graphTpl))
                    continue;

                //replace graphic section
                //newRun.Descendants<D.Graphic>().First().InnerXml cause .docx open failed !!
                int newStart = 0, newEnd = 0;
                var newGraph = GetGraphDataTpl(newRun.InnerXml, ref newStart, ref newEnd);

                srcTpl = srcTpl.Substring(0, drawStart + graphStart) +
        ...
        /// <returns></returns>
        public string GetBodyTpl(string srcText, ref int startPos, ref int endPos)
        {
            return GetRangeTpl(srcText, ref startPos, ref endPos, false, "<w:body>", "</w:body>");
        }
        ...
        /// <returns></returns>
        public string GetRowTpl(string srcText, ref int startPos, ref int endPos, int childIdx = -1)
        {
            var findMid = (childIdx < 0) ? "[!]" : $"[!{childIdx}]";
            var result = GetRangeTpl(srcText, ref startPos, ref endPos, true, "<w:tr ", "</w:tr>", findMid);
            return result.Replace(findMid, "");
        }

        public string GetDrawTpl(string srcText, ref int startPos, ref int endPos, string imageCode)
        {
            return GetRangeTpl(srcText, ref startPos, ref endPos, true, "<w:drawing>", "</w:drawing>", $"descr=\"{imageCode}\"");
        }
        public string GetGraphDataTpl(string srcText, ref int startPos, ref int endPos)
        {
            return GetRangeTpl(srcText, ref startPos, ref endPos, true, "<a:graphicData", "</a:graphicData>");
        }
        ...
        /// <returns></returns>
        private string GetRangeTpl(string srcText, ref int startPos, ref int endPos, bool hasTag, string findStart, string findEnd, string findMid = "")
        {
            ...
            var result = srcText.Substring(startPos, endPos - startPos + 1);
            if (!hasTag)
            {
                result = result.Substring(startLen, result.Length - startLen - endLen);
                startPos += startLen;
                endPos -= endLen;
            }
            return result;
        ...
        private WordImageRunDto GetImageRunDto(string filePath)
        {
            ...
            var ds = new MemoryStream(File.ReadAllBytes(filePath));
            var img = new Bitmap(ds);   //for get width/height

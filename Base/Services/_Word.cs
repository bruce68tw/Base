...
namespace Base.Services
{
    public static class _Word
    {
        ...
        /// <summary>
        /// convert template file(with row) to word file
        /// </summary>
        ...
        public static bool TplToFile(JObject row, string tplPath, string filePath, List<WordImageDto> images = null)
        {
            //check template file
            ...
            DataIntoStream(row, stream, images);
            stream.Dispose();
            return true;
        }
        ...
        public static bool TplRowToStream(string tplPath, JObject row, Stream stream, List<WordImageDto> images = null)
        {
            //check template file
            if (!File.Exists(tplPath))
            {
            ...
            return DataIntoStream(row, stream, images);
        }
        ...
        public static bool DataIntoStream(JObject row, Stream stream, List<WordImageDto> images = null, List<WordRowsDto> wordRows = null)
        {
            //stream -> docx
            using (var docx = WordprocessingDocument.Create(stream, WordprocessingDocumentType.Document))
            {
                ...
                {
                    docxText = sr.ReadToEnd();
                }

                //add image
                if (images != null)
                    DocxAddImage(mainPart, ref docxText, images);

                //replace(single row)
                //var preLen = _preTable.Length;
                foreach (var item in row)
                {
                    var key = item.Key;
                    docxText = docxText.Replace("[" + key + "]", item.Value.ToString());
                }
                ...
                if (wordRows != null)
                {
                    foreach(var wordRow in wordRows)
                    {
                        ...
                        foreach(JObject row2 in wordRow.Rows)
                        {
                            ...
                            {
                                var key = item.Key;
                                rowStr = rowStr.Replace("[" + key + "]", item.Value.ToString());
                            }
                            rowList += rowStr;
                        }

                        docxText = docxText.Substring(0, tplStart) + rowList + docxText.Substring(0, tplEnd);
                    }
                }
        ...
        /// <param name="endPos">(ref)found end pos to return</param>
        /// <param name="srcText">source text</param>
        /// <param name="midStartStr">find start string</param>
        /// <param name="midEndStr">equal to midStartStr if empty</param>
        /// <param name="startStr">find start string</param>
        /// <param name="endStr">equal to startStr if empty</param>
        /// <returns></returns>
        public static string GetRangeStr(ref int startPos, ref int endPos, string srcText, string midStartStr = "", string midEndStr = "", string startStr = "<w:tr ", string endStr = "</w:tr>")
        {
            //int midStartPos = -1, midEndPos = -1;
            if (midStartStr == "")
            {
                startPos = srcText.IndexOf(startStr);
                endPos = srcText.IndexOf(endStr);
            }
            else
            {
                var midStartPos = srcText.IndexOf(midStartStr);
                if (midStartPos < 0)
                    return "";

                var midEndPos = (midEndStr == "") ? midStartPos : srcText.IndexOf(midStartStr, midStartPos);
                if (midEndPos < 0)
                    return "";

                startPos = srcText.LastIndexOf(startStr, midStartPos);
                endPos = srcText.IndexOf(endStr, midEndPos);
            }
            ...
            //case of ok
            return srcText.Substring(startPos, endPos + endStr.Length - startPos);
        }
